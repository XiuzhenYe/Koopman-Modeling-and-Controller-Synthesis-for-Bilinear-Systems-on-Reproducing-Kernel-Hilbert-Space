...
--------------
Two-subfigure plot:
  (a) C(N) vs training size N on log-log scale with N^{-1/3} reference
  (b) 3D scatter of prediction errors inside cone bound +/- C*|(x,u)|

Uses N = 50, 100, 150, 200, 250, 300 (linspace over training set size).
C(N) = max_i |error_i| / (x_i^2 + u_i^2) — CDC Corollary 4.
"""
...
for n in n_values:
    ...
    C_n = np.max(np.abs(error[mask_q]) / xu_sq[mask_q])
    C_values.append(C_n)
...
Z_upper   =  C_full * norm_surf
Z_lower   = -C_full * norm_surf
z_max     =  C_full * np.sqrt(2)

# ── 4. Combined figure: (a) C(N) vs N  |  (b) 3D error cone ──────────────────
fig = plt.figure(figsize=(16, 6))
gs  = GridSpec(1, 2, figure=fig, width_ratios=[2, 1], wspace=0.15)

# ---- subplot (a): C(N) vs N -------------------------------------------------
ax = fig.add_subplot(gs[1])
ax.loglog(n_arr, C_values, "o-", color="steelblue", lw=2, ms=8)
ax.loglog(n_arr, C_ref,    "--", color="gray",    lw=1.8 )
...
# ---- subplot (b): 3D error cone at N=300 ------------------------------------
...
ax.xaxis.set_minor_locator(plt.NullLocator())   # removes minor ticks (6×10¹ etc.)
ax.xaxis.set_major_formatter(plt.ScalarFormatter())  # shows plain numbers, not 10^x notation
...
plt.savefig("case1_error_surface_C.png", dpi=150, bbox_inches="tight")
plt.close() 
